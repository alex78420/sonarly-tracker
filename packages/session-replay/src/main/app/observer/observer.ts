...
import { shouldInlineCSS, resolveFullCSSUrl, getCSSType } from './cssDetector.js'
...
async function parseUseEl(
  ...
  try {
    ...
    if (!href) {
      console.debug('Sonarly: xlink:href or href not found on <use>.')
      return
    }
    ...
    if (!url && symbolId) {
      ...
      `
        ...
        console.warn('Sonarly: Sprite symbol not found in the document.')
        return
      }
    }

    if (!url && !symbolId) {
      console.warn('Sonarly: Invalid xlink:href or href found on <use>.')
      return
    }
    ...
    if (!symbol) {
      console.debug('Sonarly: Symbol not found in SVG.')
      return ''
    }
    ...
    console.debug(`Sonarly: Unknown mode: ${mode}. Use "inline" or "dataurl".`)
  } catch (error) {
    console.error('Sonarly: Error processing <use> element:', error)
  }
}
...
export default abstract class Observer {
   ...
   * - true: always inline CSS (old behavior)
   * - 'auto': automatically detect if CSS should be inlined based on URL type
   * */
  private readonly inlineRemoteCss: boolean | 'auto' = 'auto'
  private readonly domParser = new DOMParser()
  constructor(
    protected readonly app: App,
    protected readonly isTopContext = false,
    options: { disableSprites: boolean, inlineRemoteCss: boolean | 'auto' } = { disableSprites: false, inlineRemoteCss: 'auto' },
  ) {
  ...
  /**
   * Determine if a CSS link should be inlined based on configuration and URL type
   * @param href - The href attribute of the link element
   * @returns true if CSS should be inlined
   */
  private shouldInlineCSSLink(href: string | null): boolean {
    if (!href) {
      return false;
    }

    // Legacy boolean mode
    if (typeof this.inlineRemoteCss === 'boolean') {
      return this.inlineRemoteCss;
    }

    // Auto detection mode
    if (this.inlineRemoteCss === 'auto') {
      const currentOrigin = window.location.origin;
      const shouldInline = shouldInlineCSS(href, currentOrigin);
  ...
  private sendNodeAttribute(id: number, node: Element, name: string, value: string | null): void {
    if (isSVGElement(node)) {
      ...
      if (isUseElement(node) && name === 'href' && !this.disableSprites) {
        parseUseEl(node, 'svgtext', this.domParser)
          .then((svgData) => {
            if (svgData) {
              this.app.send(SetNodeAttribute(id, name, `_$SONARLY_SPRITE$_${svgData}`))
            }
          })
          .catch((e: any) => {
            console.error('Sonarly: Error parsing <use> element:', e)
          })
        return
      }
    ...
    if (name === 'style' || (name === 'href' && hasTag(node, 'link'))) {
      if ('rel' in node && node.rel === 'stylesheet' && this.shouldInlineCSSLink(value)) {
        setTimeout(() => {
          inlineRemoteCss(
            // @ts-ignore
          node,
    ...
    // privateMode check removed - property doesn't exist in current Sanitizer
    this.app.attributeSender.sendSetAttribute(id, name, value)
  }
    ...
    if (isNew) {
      if (isElementNode(node)) {
        ...
        if (parentID !== undefined) {
          ...
          const cssHref = 'href' in el ? el.getAttribute('href') : null;
          if ('rel' in el && el.rel === 'stylesheet' && this.shouldInlineCSSLink(cssHref)) {
