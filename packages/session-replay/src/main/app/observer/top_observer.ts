...
import { IN_BROWSER, hasSonarlyAttribute, canAccessIframe } from '../../utils.js'
...
export enum InlineCssMode {
  ...
  All = 3,
  Auto = 4, // Automatically detect local/relative CSS and inline them
}
...
export default class TopObserver extends Observer {
  ...
  constructor(params: { app: App; options: Partial<Options> }) {
    const opts = Object.assign(
      {
        ...
        inlineCss: 4, // Use Auto mode by default (InlineCssMode.Auto)
        inlineRemoteCss: 'auto' as const, // Use auto detection by default
      },
      params.options,
    )
    super(params.app, true, { 
      disableSprites: opts.disableSprites, 
      inlineRemoteCss: opts.inlineRemoteCss as boolean | 'auto'
    })
    this.app = params.app
    this.options = opts
    // IFrames
    this.app.nodes.attachNodeCallback((node) => {
      if (
        hasTag(node, 'iframe') &&
        ((this.options.captureIFrames && !hasSonarlyAttribute(node, 'obscured')) ||
          hasSonarlyAttribute(node, 'capture'))
      ) {
