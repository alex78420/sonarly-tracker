...
export const DEFAULT_INGEST_POINT = 'https://api.sonarly.dev/ingest'
...
function getInlineOptions(mode: InlineCssMode): InlineOptions {
  switch (mode) {
    case InlineCssMode.Auto:
      return {
        inlineRemoteCss: 'auto',
        inlinerOptions: {
          forceFetch: false,
          forcePlain: false,
        },
...
export default class App {
  ...
  // For sendBeacon on beforeunload
  private beaconBatchReady = false
  private beaconBatch: Uint8Array | null = null
  ...
  constructor(
    ...
    this.inlineCss = getInlineOptions(options.inlineCss ?? 4) // Use Auto mode by default (InlineCssMode.Auto)
    ...
    if (
      ...
      console.warn(
        'Sonarly: canvas options are moving to separate key "canvas" in next update. Please update your configuration.',
      )
      options = {
        ...options,
        canvas: {
    ...
    const defaultOptions: Options = {
      revID: '',
      node_id: '__sonarly_id',
      session_token_key: '__sonarly_token',
      session_pageno_key: '__sonarly_pageno',
      session_reset_key: '__sonarly_reset',
      session_tabid_key: '__sonarly_tabid',
      local_uuid_key: '__sonarly_uuid',
  ...
  private initWorker() {
    try {
      ...
      // Track if we've already sent the final batch to avoid duplicates
      let finalBatchSent = false
      let flushInProgress = false
      
      // Unified function to send final batch (called by multiple events)
      const sendFinalBatch = (eventName: string) => {
        // Prevent duplicate sends
        if (finalBatchSent || flushInProgress) {
          console.log(`� ${eventName}: Final batch already sent/in-progress, skipping`)
          return
        }
        
        flushInProgress = true
        console.log(`🔴 ${eventName} triggered - sending final batch`)
        
        if (this.worker) {
          // Send TabClosed message
          const prevState = this.activityState
          this.activityState = ActivityState.Active
          this.send(TabClosed(this.session.getTabId()))
          this.activityState = prevState
          
          console.log(`🔴 TabClosed sent, requesting batch for sendBeacon...`)
          
          // Request worker to prepare batch for sendBeacon
          this.beaconBatchReady = false
          this.beaconBatch = null
          this.worker.postMessage('prepareBatchForBeacon')
          
          // Wait for worker to prepare batch (300ms max)
          const startWait = Date.now()
          while (!this.beaconBatchReady && Date.now() - startWait < 300) {
            // Blocking wait for worker response
          }
          const waitTime = Date.now() - startWait
          
          // If we got the batch, send with sendBeacon for guaranteed delivery
          const batch = this.beaconBatch
          if (batch !== null) {
            const token = this.session.getSessionToken()
            const ingestURL = `${this.options.ingestPoint}/v1/web/i${token ? `?token=${token}` : ''}`
            const blob = new Blob([batch], { type: 'application/octet-stream' })
            
            // Try PendingPostBeacon API first (most reliable, Chrome 121+)
            let success = false
            if ('PendingPostBeacon' in window && typeof (window as any).PendingPostBeacon === 'function') {
              try {
                const beacon = new (window as any).PendingPostBeacon(ingestURL, {
                  method: 'POST',
                  body: blob,
                })
                success = true
                console.log(`🟢 PendingPostBeacon queued successfully (${blob.size} bytes)`)
                finalBatchSent = true
              } catch (e) {
                console.warn(`⚠️ PendingPostBeacon failed, falling back to sendBeacon:`, e)
              }
            }
            
            // Fallback to sendBeacon
            if (!success) {
              success = navigator.sendBeacon(ingestURL, blob)
              console.log(`${success ? '�' : '�🔴'} sendBeacon ${success ? 'SUCCESS' : 'FAILED'} after ${waitTime}ms wait, batch size: ${blob.size} bytes (${eventName})`)
              if (success) {
                finalBatchSent = true
              }
            }
          } else {
            console.log(`🔴 No batch received from worker after ${waitTime}ms`)
          }
        }
      ...
      // Handler for beforeunload (most common)
      const handleBeforeUnload = () => {
        sendFinalBatch('beforeunload')
      }
      
      // Handler for unload (backup for beforeunload)
      const handleUnload = () => {
        sendFinalBatch('unload')
      }
      
      // Handler for pagehide (Mobile Safari, iOS)
      const handlePageHide = () => {
        sendFinalBatch('pagehide')
      }
      
      // Preventive flush when tab becomes hidden (NOT final - user may return)
      const handleVisibilityChange = () => {
        if (document.hidden && this.worker && !finalBatchSent) {
          console.log('🟡 Tab hidden - preventive flush (NO TabClosed, session stays alive)')
          // Just flush current batch WITHOUT sending TabClosed
          // Backend will timeout session after 2m30 of inactivity if user never returns
          this.worker.postMessage('forceFlushBatch')
          alertWorker()
        } else if (!document.hidden && this.worker) {
          console.log('🟢 Tab visible again - session continues normally')
          // Session continues, no action needed
        }
      }
      
      // Register all unload events for maximum reliability (90%+ coverage)
      // Priority order: pagehide > beforeunload > unload > visibilitychange
      
      // 1. pagehide - Most reliable on mobile (iOS Safari)
      this.attachEventListener(window, 'pagehide', handlePageHide, false)
      
      // 2. beforeunload - Most common on desktop browsers
      this.attachEventListener(window, 'beforeunload', handleBeforeUnload, false)
      
      // 3. unload - Backup for older browsers
      this.attachEventListener(window, 'unload', handleUnload, false)
      
      // 4. visibilitychange - Preventive flush (not final)
      this.attachEventListener(document, 'visibilitychange', handleVisibilityChange, false)
      
      // 5. Keep existing mouseleave for activity tracking
  ...
  private handleWorkerMsg(data: FromWorkerData) {
    ...
    if (data === 'a_stop') {
    ...
    } else if (data.type === 'batchReady') {
      // Handle batch prepared for sendBeacon on page unload
      this.beaconBatch = data.batch
      this.beaconBatchReady = true
      ...
      if (batchSize > this.compressionThreshold) {
        gzip(data.batch, { mtime: 0 }, (err, result) => {
          if (err) {
            this.debug.error('Sonarly compression error:', err)
  ...
  private _debug(context: string, e: any) {
    ...
    this.debug.error('Sonarly error: ', context, e)
  }
  ...
  getSessionURL(options?: { withCurrentTime?: boolean }): string | undefined {
    ...
    if (!projectID || !sessionID) {
      this.debug.error('Sonarly error: Unable to build session URL')
      return undefined
    }
    const ingest = this.options.ingestPoint
    const isSaas = /api\.sonarly\.com/.test(ingest)

    const projectPath = isSaas ? 'https://app.sonarly.dev/ingest' : ingest
  ...
  private async _start(
    ...
    if (
      ...
      const reason =
        'Sonarly: trying to call `start()` on the instance that has been started already.'
      return Promise.resolve(UnsuccessfulStart(reason))
    }
    this.activityState = ActivityState.Starting
    if (!isColdStart) {
    ...
    this.debug.log(
      'Sonarly: starting session; need new session id?',
      isNewSession,
      'session token: ',
      sessionToken,
    )
  ...
  stop(stopWorker = true): void {
    if (this.activityState !== ActivityState.NotActive) {
      try {
        ...
        this.debug.log('Sonarly tracking stopped.')
      }
    }
  }
}
