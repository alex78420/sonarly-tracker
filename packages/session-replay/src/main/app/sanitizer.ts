...
import { stars, hasSonarlyAttribute } from '../utils.js'
...
export default class Sanitizer {
  ...
  handleNode(id: number, parentID: number, node: Node) {
    if (
      ...
      (isElementNode(node) &&
        (hasSonarlyAttribute(node, 'masked') || hasSonarlyAttribute(node, 'obscured')))
    ) {
    ...
    if (
      ...
      (isElementNode(node) &&
        (hasSonarlyAttribute(node, 'htmlmasked') || hasSonarlyAttribute(node, 'hidden')))
    ) {
      this.hidden.add(id)
    }
