...
export default function (
  ...
  app.debug.log('Sonarly: attaching axios spy to instance', instance)

  function captureResponseData(axiosResponseObj: AxiosResponse) {
    app.debug.log('Sonarly: capturing axios response data', axiosResponseObj)
    ...
    if (!reqResInfo) {
      app.debug.log('Sonarly: empty request/response info, skipping')
      return
    }
    const requestStart = axiosResponseObj.config.__sonarly_timing
    const duration = performance.now() - requestStart

    app.debug.log('Sonarly: final req object', reqResInfo)
    app.send(
      NetworkRequest(
        'xhr',
        String(method),
  ...
  function getStartTime(config: InternalAxiosRequestConfig) {
    app.debug.log('Sonarly: capturing API request', config)
    config.__sonarly_timing = performance.now()
    if (opts.sessionTokenHeader) {
      const header =
        typeof opts.sessionTokenHeader === 'string'
          ? opts.sessionTokenHeader
          : 'X-Sonarly-Session-Token'
  ...
  function captureNetworkError(error: Record<string, any>) {
    app.debug.log('Sonarly: capturing API request error', error)
  ...
  function logRequestError(ev: any) {
    app.debug.log('Sonarly: failed API request, skipping', ev)
  }
