...
export const DOCS_HOST = 'https://docs.sonarly.dev'
...
export function deprecationWarn(nameOfFeature: string, useInstead: string, docsPath = '/'): void {
  ...
  console.warn(
    `Sonarly: ${nameOfFeature} is deprecated. ${
...
export function getLabelAttribute(e: Element): string | null {
  let value = e.getAttribute('data-sonarly-label')
  if (value !== null) {
    return value
  }
  return value
...
export function hasSonarlyAttribute(e: Element, attr: string): boolean {
  const newName = `data-sonarly-${attr}`
  return e.hasAttribute(newName)
}
...
export function createEventListener(
  ...
  try {
    ...
    console.error(
      ...
      `Sonarly: ${msg}; if this error is caused by an IframeObserver, ignore it`,
      event,
      target,
    )
  }
