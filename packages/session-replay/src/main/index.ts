...
function processOptions(obj: any): obj is Options {
  if (obj == null) {
    console.error(
      `Sonarly: invalid options argument type. Please, check documentation on ${DOCS_HOST}${DOCS_SETUP}`,
    )
    return false
  }
  if (typeof obj.projectKey !== 'string') {
    if (typeof obj.projectKey !== 'number') {
      if (typeof obj.projectID !== 'number') {
        // Back compatability
        console.error(
          `Sonarly: projectKey is missing or wrong type (string is expected). Please, check ${DOCS_HOST}${DOCS_SETUP} for more information.`,
        )
        return false
      } else {
      ...
      console.warn('Sonarly: projectKey is expected to have a string type.')
...
export default class API {
  ...
  constructor(public readonly options: Partial<Options>) {
    ...
    if (
      (window as any).__SONARLY__ ||
      (!this.crossdomainMode && inIframe() && canAccessTop() && (window.top as any).__SONARLY__)
    ) {
      console.error('Sonarly: one tracker instance has been initialised already')
      return
    }
    if (!options.__DISABLE_SECURE_MODE && location.protocol !== 'https:') {
      console.error(
        'Sonarly: Your website must be publicly accessible and running on SSL in order for Sonarly to properly capture and replay the user session. You can disable this check by setting `__DISABLE_SECURE_MODE` option to `true` if you are testing in localhost. Keep in mind, that asset files on a local machine are not available to the outside world. This might affect tracking if you use css files.',
      )
      return
    }
    ...
    if (failReason.length > 0) {
      ...
      console.error(
        `Sonarly: browser doesn't support API required for tracking or doNotTrack is set to 1. Reason: ${missingApi}`,
      )
    ...
    Selection(app)
    ;(window as any).__SONARLY__ = this
      ...
      app.attachStartCallback(() => {
        ...
        window.open = function (...args) {
          ...
          if (options.resetTabOnWindowOpen) {
            sessStorage.removeItem(options.session_tabid_key || '__sonarly_tabid')
          }
          app.resetNextPageSession(false)
          sessStorage.setItem(options.session_tabid_key || '__sonarly_tabid', tabId)
  ...
  browserEnvCheck() {
    if (!IN_BROWSER) {
      console.error(
        `Sonarly: you are trying to start Tracker on a node.js environment. If you want to use Sonarly with SSR, please, use componentDidMount or useEffect API for placing the \`tracker.start()\` line. Check documentation on ${DOCS_HOST}${DOCS_SETUP}`,
      )
      return false
    }
    return true
