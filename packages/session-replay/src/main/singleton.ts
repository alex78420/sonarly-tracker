...
class TrackerSingleton {
   ...
   * https://docs.sonarly.dev/en/sdk/constructor/#initialization-options
   */
  configure(options: Partial<Options>): void {
    ...
    if (this.isConfigured) {
      console.warn(
        'Sonarly: Tracker is already configured. You should only call configure once.',
      )
      return
    }

    if (!options.projectKey) {
      console.error('Sonarly: Missing required projectKey option')
      return
    }
  ...
  private ensureConfigured() {
    if (!this.isConfigured && IN_BROWSER) {
      console.warn(
        'Sonarly: Tracker must be configured before use. Call tracker.configure({projectKey: "your-project-key"}) first.',
      )
      return false
    }
    return true
