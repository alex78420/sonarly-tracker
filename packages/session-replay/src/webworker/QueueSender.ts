...
export default class QueueSender {
  ...
  private lastBatch: Uint8Array | null = null  // Store last batch for sendBeacon fallback
  ...
  push(batch: Uint8Array): void {
    this.lastBatch = batch  // Store for potential sendBeacon fallback
  ...
  private sendBatch(batch: Uint8Array, isCompressed?: boolean, batchNum?: string | number): void {
    ...
    const batchBlob = new Blob([batch.buffer as ArrayBuffer], { type: 'application/octet-stream' })
  ...
  getLastBatch(): Uint8Array | null {
    // Return the last batch prepared (for sendBeacon on page unload)
    return this.lastBatch
  }

  forceFlush() {
    // CRITICAL: Force immediate send of queued batches on page unload
    // Bypass busy state to ensure TabClosed message is sent
    this.busy = false
    this.sendNext()
  }
